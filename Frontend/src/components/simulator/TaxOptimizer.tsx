  ...
  const renderTaxInput = (
    ...
    <div>
      <div className="flex justify-between items-center mb-1.5">
        <label className="block text-xs md:text-sm font-semibold text-gray-700 dark:text-gray-300">{label}</label>
        {hint && <span className="text-[11px] text-gray-400 dark:text-gray-500 font-medium">{hint}</span>}
      </div>
      <div className="relative">
        {unit === '฿' && <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm">฿</span>}
        <input
          type="number"
          placeholder={placeholder || "0"}
          className={`w-full bg-gray-50 dark:bg-gray-900/60 border border-gray-200 dark:border-gray-700 rounded-xl py-2 ${unit === '฿' ? 'pl-8 pr-3' : 'px-3'} text-sm font-semibold focus:ring-2 focus:ring-blue-500 focus:border-blue-500 transition-all outline-none`}
          value={taxDeductions[field] as string}
          onChange={(e) => setTaxDeductions({ ...taxDeductions, [field]: e.target.value })}
        />
        {unit !== '฿' && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm">{unit}</span>}
      </div>
    </div>
  );
  ...
  const CustomTooltip = ({ active, payload, label }: any) => {
    ...
    return (
      <div className="bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl p-3 shadow-lg text-xs">
        <div className="font-bold text-gray-900 dark:text-white mb-2">พ.ศ. {label}</div>
        {payload.map((p: any, i: number) => (
          <div key={i} className="flex items-center gap-2 mb-1">
            <div className="w-2.5 h-2.5 rounded-full" style={{ background: p.color }} />
            <span className="text-gray-600 dark:text-gray-400">{p.name}:</span>
            <span className="font-mono font-bold text-gray-900 dark:text-white">฿{Number(p.value).toLocaleString()}</span>
          </div>
        ))}
      </div>
    );
          ...
          <ul key={bi} className="space-y-2">
            {lines.map((l, li) => (
              <li key={li} className="flex items-start gap-2.5">
                <span className="w-1.5 h-1.5 rounded-full bg-indigo-400 mt-[9px] shrink-0"></span>
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 pb-1">
        <div>
          <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-900 dark:text-white m-0 pb-1">
            Tax <span className="bg-clip-text text-transparent bg-gradient-to-r from-blue-600 to-indigo-600">Optimizer</span>
          </h1>
        ...
        <div className="flex w-full sm:w-auto bg-gray-100 dark:bg-gray-800/80 p-1 rounded-xl border border-gray-200/60 dark:border-gray-700/60">
          <button
            ...
            className={`flex-1 sm:flex-initial px-4 sm:px-5 py-2.5 rounded-lg text-sm font-bold transition-all duration-200 flex items-center justify-center gap-2 ${
      ...
      {taxSubTab === 'deductions' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 space-y-8 p-2 md:p-4">
            ...
            <section className="rounded-2xl border border-purple-200/80 dark:border-purple-900/60 bg-white/70 dark:bg-gray-900/20 p-5 md:p-6 shadow-sm">
              ...
              <div className="flex flex-wrap items-end gap-3 mt-5">
                <div>
                  <label className="block text-xs font-bold text-gray-500 mb-1.5">ปีภาษี (พ.ศ.)</label>
                  <select
                    className="bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-lg py-2.5 px-3 text-sm font-bold focus:ring-2 focus:ring-blue-500 outline-none min-w-[120px]"
                ...
                <button
                  ...
                  className="px-5 py-2.5 bg-blue-600 hover:bg-blue-700 disabled:bg-gray-400 text-white text-sm font-bold rounded-lg transition-all duration-200 shadow-sm hover:shadow-md disabled:cursor-not-allowed"
                >
            ...
            <section className="space-y-6">
              <div className="flex items-center justify-between">
                ...
                <span className="text-xs font-bold px-3 py-1.5 rounded-full bg-purple-50 dark:bg-purple-900/30 text-purple-700 dark:text-purple-300 border border-purple-200 dark:border-purple-800">
              ...
              <div className="space-y-5">
                ...
                <div className="rounded-2xl border border-purple-200/80 dark:border-purple-900/60 bg-white/80 dark:bg-gray-900/40 p-5 md:p-6 shadow-sm transition-all hover:border-purple-300 dark:hover:border-purple-800">
                  <div 
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-gray-900 dark:text-white text-base">กลุ่มประกันสังคมและประกันชีวิต</div>
                        <div className="text-xs text-gray-500">ประกันสังคม ประกันสุขภาพ ประกันชีวิต และประกันบำนาญ</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <i className={`fi ${taxAccordions.insurance ? 'fi-sr-angle-up' : 'fi-sr-angle-down'} text-gray-400 text-sm`}></i>
                    </div>
                  </div>
                  {taxAccordions.insurance && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-5 pt-4 border-t border-gray-100 dark:border-gray-800">
                ...
                <div className="rounded-2xl border border-emerald-200/80 dark:border-emerald-900/60 bg-white/80 dark:bg-gray-900/40 p-5 md:p-6 shadow-sm transition-all hover:border-emerald-300 dark:hover:border-emerald-800">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-gray-900 dark:text-white text-base">กลุ่มการออมและการลงทุนเพื่อการเกษียณ</div>
                        <div className="text-xs text-gray-500">PVD, SSF, RMF, ThaiESG, กอช. และวิสาหกิจเพื่อสังคม</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <i className={`fi ${taxAccordions.investment ? 'fi-sr-angle-up' : 'fi-sr-angle-down'} text-gray-400 text-sm`}></i>
                    </div>
                  </div>
                ...
                <div className="rounded-2xl border border-pink-200/80 dark:border-pink-900/60 bg-white/80 dark:bg-gray-900/40 p-5 md:p-6 shadow-sm transition-all hover:border-pink-300 dark:hover:border-pink-800">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-gray-900 dark:text-white text-base">กลุ่มครอบครัวและการดูแล</div>
                        <div className="text-xs text-gray-500">คู่สมรส บุตร บิดามารดา และค่าฝากครรภ์คลอดบุตร</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <i className={`fi ${taxAccordions.family ? 'fi-sr-angle-up' : 'fi-sr-angle-down'} text-gray-400 text-sm`}></i>
                    </div>
                  </div>
                  {taxAccordions.family && (
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4 items-end mt-5 pt-4 border-t border-gray-100 dark:border-gray-800">
                      <div className="flex items-center h-[42px] px-3.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900/60">
                ...
                <div className="rounded-2xl border border-amber-200/80 dark:border-amber-900/60 bg-white/80 dark:bg-gray-900/40 p-5 md:p-6 shadow-sm transition-all hover:border-amber-300 dark:hover:border-amber-800">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-gray-900 dark:text-white text-base">มาตรการรัฐและกระตุ้นเศรษฐกิจ</div>
                        <div className="text-xs text-gray-500">Easy E-Receipt และท่องเที่ยวเมืองรอง</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <i className={`fi ${taxAccordions.stimulus ? 'fi-sr-angle-up' : 'fi-sr-angle-down'} text-gray-400 text-sm`}></i>
                    </div>
                  </div>
                ...
                <div className="rounded-2xl border border-orange-200/80 dark:border-orange-900/60 bg-white/80 dark:bg-gray-900/40 p-5 md:p-6 shadow-sm transition-all hover:border-orange-300 dark:hover:border-orange-800">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-gray-900 dark:text-white text-base">กลุ่มที่อยู่อาศัย</div>
                        <div className="text-xs text-gray-500">ดอกเบี้ยเงินกู้ยืมเพื่อซื้อที่อยู่อาศัยและค่าซ่อมแซม</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <i className={`fi ${taxAccordions.housing ? 'fi-sr-angle-up' : 'fi-sr-angle-down'} text-gray-400 text-sm`}></i>
                    </div>
                  </div>
                ...
                <div className="rounded-2xl border border-rose-200/80 dark:border-rose-900/60 bg-white/80 dark:bg-gray-900/40 p-5 md:p-6 shadow-sm transition-all hover:border-rose-300 dark:hover:border-rose-800">
                  ...
                  >
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <div className="font-bold text-gray-900 dark:text-white text-base">กลุ่มเงินบริจาค</div>
                        <div className="text-xs text-gray-500">การศึกษา โรงพยาบาล บริจาคทั่วไป และพรรคการเมือง</div>
                      </div>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <i className={`fi ${taxAccordions.donation ? 'fi-sr-angle-up' : 'fi-sr-angle-down'} text-gray-400 text-sm`}></i>
                    </div>
                  </div>
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-6">
            ...
            <div className="bg-[var(--bg-main)] rounded-2xl border border-[var(--border)] shadow-sm overflow-hidden flex flex-col">
              <div className="p-5 border-b border-[var(--border)] bg-gray-50 dark:bg-gray-900/50 flex items-center justify-between font-bold text-gray-900 dark:text-white">
                <div className="flex items-center gap-2">
              ...
              <div className="p-5 space-y-3 text-sm">
                ...
                <div className="mt-5 bg-blue-50 dark:bg-blue-900/20 rounded-xl p-4 border border-blue-100 dark:border-blue-900/50">
            ...
            <div className="bg-[var(--bg-main)] rounded-2xl border border-[var(--border)] shadow-sm overflow-hidden flex flex-col">
              <div className="p-5 border-b border-[var(--border)] bg-gray-50 dark:bg-gray-900/50 flex items-center justify-between font-bold text-gray-900 dark:text-white">
                ...
                <span className="text-[11px] font-semibold text-gray-400">มาตรา 47 ทวิ</span>
              </div>
              <div className="p-5 space-y-4 text-sm">
                
                {/* Dividend Input */}
                <div>
                  <label className="block text-xs font-bold text-gray-700 dark:text-gray-300 mb-1.5">เงินปันผลรับรวมทั้งปี (ก่อนหักภาษี 10%)</label>
                  <div className="relative">
                    <span className="absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm">฿</span>
                    <input
                      type="number"
                      className="w-full bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-xl py-2.5 pl-8 pr-3 text-sm font-bold focus:ring-2 focus:ring-emerald-500 outline-none"
                ...
                <div>
                  <div className="flex justify-between items-center mb-1.5">
                    <label className="block text-xs font-bold text-gray-700 dark:text-gray-300">อัตราภาษีนิติบุคคลของหุ้น</label>
                    <span className="text-[11px] text-emerald-600 dark:text-emerald-400 font-bold">
                      {(corporateTaxRate * 100).toFixed(0)}%
                    </span>
                  </div>
                  <select
                    className="w-full bg-gray-50 dark:bg-gray-900/50 border border-gray-200 dark:border-gray-700 rounded-xl py-2 px-3 text-xs font-bold focus:ring-2 focus:ring-emerald-500 outline-none"
                  ...
                  <div className="text-[11px] text-gray-500 mt-1.5 flex items-center justify-between">
                    ...
                    <strong className="text-gray-800 dark:text-gray-200 font-mono">{(taxResult.marginalRate * 100).toFixed(0)}%</strong>
                  </div>
                </div>

                {/* Real Detailed Calculations */}
                <div className="space-y-2 pt-3 border-t border-gray-100 dark:border-gray-800 text-xs md:text-sm">
                  <div className="flex justify-between items-center text-gray-500">
                    <span>ภาษีหัก ณ ที่จ่าย (10%)</span>
                    <span className="font-mono text-gray-800 dark:text-gray-200">฿{fmt(dividendResult.withholdingTax)}</span>
                  </div>
                  <div className="flex justify-between items-center text-gray-500">
                  ...
                  <div className="flex justify-between items-center text-gray-500">
                    ...
                    <span className="font-mono text-gray-800 dark:text-gray-200">฿{fmt(dividendResult.grossedDividend)}</span>
                  </div>
                  <div className="flex justify-between items-center font-semibold text-gray-700 dark:text-gray-300 pt-1 border-t border-dashed border-gray-100 dark:border-gray-800">
            ...
            <div className="bg-gradient-to-br from-indigo-50/70 via-white to-purple-50/70 dark:from-gray-900 dark:via-gray-900/90 dark:to-purple-950/30 rounded-2xl border border-indigo-200/80 dark:border-indigo-900/60 shadow-sm p-5 flex flex-col items-center text-center gap-3">
              ...
              <div className="font-bold text-gray-900 dark:text-white text-sm">AI วิเคราะห์ภาษีเชิงลึก</div>
              ...
              <button
                onClick={() => setTaxSubTab('ai-analysis')}
                className="w-full px-4 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white text-xs font-bold rounded-xl transition-all shadow-sm hover:shadow flex items-center justify-center gap-1.5"
              >
      ...
      {taxSubTab === 'ai-analysis' && (
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
          ...
          <div className="lg:col-span-8 p-2 md:p-4">
            <div className="rounded-2xl border border-indigo-200/80 dark:border-indigo-900/60 bg-gradient-to-br from-indigo-50/70 via-white to-purple-50/70 dark:from-gray-900 dark:via-gray-900/90 dark:to-purple-950/30 shadow-sm overflow-hidden">

              {/* Card header */}
              <div className="p-5 border-b border-indigo-100 dark:border-indigo-900/50 flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
                ...
                <button
                  ...
                  className="px-3 py-1.5 bg-indigo-600 hover:bg-indigo-700 disabled:bg-gray-300 text-white text-xs font-bold rounded-lg transition-all shadow-sm hover:shadow flex items-center gap-1.5 disabled:cursor-not-allowed whitespace-nowrap"
                >
                  {isTaxAdviceLoading ? (
                    <>
              ...
              <div className="p-5 md:p-6">
                {isTaxAdviceLoading ? (
                  <div className="space-y-4 py-10 text-center">
                    ...
                    <div className="text-xs text-gray-400">ใช้เวลาประมวลผลประมาณ 10-30 วินาที</div>
                  </div>
                ) : taxAdvice ? (
                  <div className="space-y-5">
                    <div className="text-sm text-gray-700 dark:text-gray-300 leading-relaxed space-y-3 bg-white/70 dark:bg-gray-900/40 p-5 rounded-xl border border-indigo-100 dark:border-indigo-900/30">
                    ...
                    <div className="flex justify-center">
                      <button
                        ...
                        className="px-5 py-2.5 bg-white dark:bg-gray-800 hover:bg-indigo-50 dark:hover:bg-gray-700 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-800 text-xs font-bold rounded-xl transition-all flex items-center gap-1.5 shadow-sm"
                      >
                  ...
                  <div className="text-center py-8 px-4">
                    ...
                    {/* Title + description */}
                    <div className="text-lg font-extrabold text-gray-900 dark:text-white mb-2">
                      วางแผนภาษีอย่างชาญฉลาดด้วย AI
                    </div>
                    <div className="text-sm text-gray-500 dark:text-gray-400 max-w-md mx-auto mb-6 leading-relaxed">
                    ...
                    {annualIncome ? (
                      <button
                        onClick={analyzeTaxWithAI}
                        className="px-8 py-3 bg-gradient-to-r from-indigo-600 to-purple-600 hover:from-indigo-700 hover:to-purple-700 text-white text-sm font-bold rounded-xl transition-all shadow-md hover:shadow-lg hover:-translate-y-0.5"
                      >
          ...
          <div className="lg:col-span-4 space-y-6 lg:sticky lg:top-6">
            <div className="bg-[var(--bg-main)] rounded-2xl border border-[var(--border)] shadow-sm overflow-hidden">
              <div className="p-5 border-b border-[var(--border)] bg-gray-50 dark:bg-gray-900/50 flex items-center justify-between font-bold text-gray-900 dark:text-white">
                <div className="flex items-center gap-2">
                  <i className="fi fi-sr-database text-indigo-500"></i> ข้อมูลที่ใช้วิเคราะห์
                </div>
                <span className="text-xs font-bold px-2.5 py-1 rounded-lg bg-indigo-100 dark:bg-indigo-900/40 text-indigo-700 dark:text-indigo-300">
                  พ.ศ. {selectedTaxYear}
                </span>
              </div>
              <div className="p-5 space-y-3 text-sm">
                ...
                <div className="flex justify-between items-center text-gray-500">
                  ...
                  <span className="font-mono font-bold text-gray-900 dark:text-white">฿{fmt(taxResult.taxWithDeductions)}</span>
                </div>
                ...
                <div className="flex justify-between items-center text-gray-500 pt-1 border-t border-gray-100 dark:border-gray-800">
            ...
            {/* Hint card */}
            <div className="bg-indigo-50/70 dark:bg-indigo-900/20 rounded-2xl border border-indigo-100 dark:border-indigo-900/50 p-4 flex items-start gap-3">
              <i className="fi fi-sr-info text-indigo-500 mt-0.5"></i>
                    ...
                    >
                      ...
                      >
                        <div className="flex items-center gap-4">
                          {/* Year badge */}
                          <div className="w-14 h-14 rounded-xl bg-gradient-to-br from-blue-500 to-cyan-500 flex flex-col items-center justify-center text-white shrink-0 shadow-sm">
                          ...
                          {/* Summary info */}
                          <div>
                            <div className="text-xs text-gray-500 dark:text-gray-400 mb-0.5">ภาษีที่ต้องจ่าย</div>
                            <div className="text-xl font-extrabold font-mono text-gray-900 dark:text-white">
                              ฿{fmt(Math.round(record.taxWithDeductions))}
                            </div>
                            <div className="flex gap-4 mt-1 text-[11px] text-gray-500 dark:text-gray-400">
                              <span>รายได้: <strong className="text-gray-700 dark:text-gray-300 font-mono">฿{fmt(Math.round(record.annualIncome))}</strong></span>
                        ...
                        <div className="flex items-center gap-2">
                          ...
                          {/* Expand arrow */}
                          <i className={`fi ${isExpanded ? 'fi-sr-angle-up' : 'fi-sr-angle-down'} text-gray-400 text-sm transition-transform duration-200`} />
                        </div>
                      </div>

                      {/* Expanded detail (dropdown) */}
                      {isExpanded && (
                        <div className="border-t border-gray-100 dark:border-gray-800 bg-gray-50/50 dark:bg-gray-900/30 p-4 md:p-5 animate-[fadeIn_0.2s_ease]">
                          {/* Tax breakdown */}
                          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-5">
                            <div className="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                              <div className="text-[10px] text-gray-500 font-semibold mb-1">เงินได้สุทธิ</div>
                              <div className="font-mono font-bold text-sm text-gray-900 dark:text-white">฿{fmt(Math.round(record.netIncome))}</div>
                            </div>
                            <div className="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                              <div className="text-[10px] text-gray-500 font-semibold mb-1">ภาษีก่อนลดหย่อน</div>
                              <div className="font-mono font-bold text-sm text-red-600">฿{fmt(Math.round(record.taxWithoutDeductions))}</div>
                            </div>
                            <div className="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                              <div className="text-[10px] text-gray-500 font-semibold mb-1">ภาษีหลังลดหย่อน</div>
                              <div className="font-mono font-bold text-sm text-gray-900 dark:text-white">฿{fmt(Math.round(record.taxWithDeductions))}</div>
                            </div>
                            <div className="bg-white dark:bg-gray-800 rounded-lg p-3 border border-gray-100 dark:border-gray-700">
                              <div className="text-[10px] text-gray-500 font-semibold mb-1">ลดหย่อนรวม</div>
                          ...
                          {activeDeductions.length > 0 && (
                            <div>
                              <div className="text-xs font-bold text-gray-500 dark:text-gray-400 mb-3 uppercase tracking-wider">
                                รายการลดหย่อนที่ใช้
                              </div>
                                ...
                                {activeDeductions.map(([key, val]) => {
                                  ...
                                  return (
                                    <div key={key} className="flex justify-between items-center text-xs py-1.5 px-2 rounded bg-white dark:bg-gray-800 border border-gray-100 dark:border-gray-700">
                                      <span className="text-gray-600 dark:text-gray-400">{DEDUCTION_LABELS[key] || key}</span>
